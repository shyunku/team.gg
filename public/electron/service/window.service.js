...
import WindowPropertyFactory from "../util/WindowPropertyFactory.js";
import Constants from "../modules/constants.js";
import electronRemote from "@electron/remote/main/index.js";
import AlertPopupConstants from "../constants/AlertPopup.constants.js";
import url from "url";
...
class WindowService {
  ...
  createMainWindow(overlapWindowProperty = {}) {
    let windowProperty = new WindowPropertyFactory()
      .windowType(Constants.WindowType.Modeless)
      .show(false)
      .center(true)
      .frame(false)
      .minWidth(900)
  ...
  createModalWindow(browserId, url, windowProperty, parameter) {
    ...
    if (browserId) {
      ...
      currentlyFocusedWindowCenterPos = this.getWindowCenterPos(currentlyFocusedWindow);
      defaultWindowFactory.parent(currentlyFocusedWindow);
    }

    const defaultWindowProperty = defaultWindowFactory.build();
    let newWindow = this.invokeWindow("/modal" + url, defaultWindowProperty, parameter);

    if (browserId) {
      this.setWindowCenterPos(newWindow, currentlyFocusedWindowCenterPos.x, currentlyFocusedWindowCenterPos.y);
    }
  ...
  async createUpdaterWindow(overlapWindowProperty) {
    ...
    return new Promise((resolve, reject) => {
      try {
        let window = this.invokeWindow("/update-checker", windowProperty, null, (window) => {
          resolve(window);
        });
  ...
  createAlertPopupWindow(url, windowProperty, parameter) {
    ...
    let newWindow = this.invokeWindow("/alert-popup" + url, defaultWindowProperty, parameter);
    return newWindow;
  }
